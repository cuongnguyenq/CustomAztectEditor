...
class BlockFormatter(editor: AztecText,
    ...
    private var alignmentSpanLasted: ITextFormat = AztecTextFormat.FORMAT_NONE
    ...
    fun toggleOrderedList() {
        ...
        if (alignmentSpanLasted != AztecTextFormat.FORMAT_NONE) {
            applyTextAlignment(alignmentSpanLasted)
        }
    }
    ...
    private fun removeTextAlignment(textFormat: ITextFormat) {
    ...
    private fun removeBlockStyle(textFormat: ITextFormat) {
        removeBlockStyle(textFormat, selectionStart, selectionEnd, makeBlock(textFormat, 0).map { it -> it.javaClass })
    }

    private fun <T : IAztecBlockSpan> removeEntireBlock(type: Class<T>) {
    ...
    private fun makeBlock(textFormat: ITextFormat, nestingLevel: Int, attrs: AztecAttributes = AztecAttributes()): List<IAztecBlockSpan> {
    ...
    private fun makeBlockSpan(textFormat: ITextFormat, nestingLevel: Int, attrs: AztecAttributes = AztecAttributes()): IAztecBlockSpan {
        return when (textFormat) {
    ...
    private fun getTopBlockDelimiters(start: Int, end: Int): List<Int> {
     ...
     */
    private fun getBoundsOfText(editable: Editable, selectionStart: Int, selectionEnd: Int): IntRange {
    ...
    private fun applyTextAlignment(textFormat: ITextFormat, start: Int = selectionStart, end: Int = selectionEnd) {
        ...
        alignmentSpanLasted = textFormat
        if (spans.isNotEmpty()) {
    ...
    private fun applyBlockStyle(blockElementType: ITextFormat, start: Int = selectionStart, end: Int = selectionEnd) {
        if (editableText.isEmpty()) {
            editableText.append("" + Constants.END_OF_BUFFER_MARKER)
        }
    ...
    private fun containsBlockElement(textFormat: ITextFormat, index: Int, text: Editable, nestingLevel: Int): Boolean {
    ...
    private fun containsHeading(textFormat: ITextFormat, selStart: Int = selectionStart, selEnd: Int = selectionEnd): Boolean {
        val lines = TextUtils.split(editableText.toString(), "\n")
        val list = ArrayList<Int>()
    ...
    private fun containsOtherHeadings(textFormat: ITextFormat, selStart: Int = selectionStart, selEnd: Int = selectionEnd): Boolean {
    ...
    private fun containsHeadingOnly(textFormat: ITextFormat, selStart: Int = selectionStart, selEnd: Int = selectionEnd): Boolean {
    ...
    private fun containsPreformat(index: Int): Boolean {
        val lines = TextUtils.split(editableText.toString(), "\n")
        if (index < 0 || index >= lines.size) {
            return false
        }
    ...
    private fun switchListType(listTypeToSwitchTo: ITextFormat, start: Int = selectionStart, end: Int = selectionEnd) {
    ...
    private fun switchHeaderType(headerTypeToSwitchTo: ITextFormat, start: Int = selectionStart, end: Int = selectionEnd) {
    ...
    private fun switchHeadingToPreformat(start: Int = selectionStart, end: Int = selectionEnd) {
        var spans = editableText.getSpans(start, end, AztecHeadingSpan::class.java)
        if (start == end && spans.size > 1) {
            spans = spans.filter { editableText.getSpanStart(it) == start }.toTypedArray()
        }
    ...
    private fun switchPreformatToHeading(headingTextFormat: ITextFormat, start: Int = selectionStart, end: Int = selectionEnd) {
